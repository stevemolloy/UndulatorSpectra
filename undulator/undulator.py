from .ebeam import sig, sigp, beamgamma
...
class Undulator:
    def __init__(self, insdev, beam):
        insdev = namedtuple('insdev', insdev)
        self.insdev = insdev(*insdev.values())
        beam = namedtuple('beam', beam)
        self.beam = beam(*beam.valuies())

    def lamda_n(self, n=1, theta=0):
        gamma = beamgamma(beam.energy)
        unscaled = insdev.lamda_w / (2 * n * gamma**2)
    ...
    def source_spot_y(insdev, beam, n=1, theta=0):
        energy = beam.energy
        gamma = beamgamma(energy)
        osc_amplitude = insdev.lamda_w*insdev.Kmax / (2*pi*gamma)
